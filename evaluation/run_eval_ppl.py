...
def parse_args():
    ...
    parser.add_argument(
        "--calibrate",
        action="store_true",
        help="If passed, will apply PMI_dc_probs to prediction instead of input_log_probs",
    )
...
def main():
    ...
    if args.dataset_name == "anli":
        prompts = DatasetTemplates(args.dataset_name)
    else:
    ...
    if "," in args.template_name:
        ...
        templates = {template_name: prompts[template_name] for template_name in template_names}
    else:
        templates = {args.template_name: prompts[args.template_name]}
    
    def get_prompt_inputs(template, eval_dataset, device):
        empty_input = {"":""}
        prompt_text, _ = template.apply(empty_input)
        ex_answer_choices = template.get_answer_choices_list(empty_input)
        prompt_inputs = tokenizer.encode_plus(prompt_text)
        prompt_inputs = {
            k: [v for _ in range(len(ex_answer_choices))]
            for k, v in prompt_inputs.items()
        }
        prompt_labels = tokenizer.pad({"input_ids": eval_dataset['labels'][0]})
        prompt_inputs['labels'] = prompt_labels['input_ids']
        prompt_inputs = {k: torch.tensor(v).to(device) for k, v in prompt_inputs.items()}
    ...
    for template_name, template in templates.items():
        ...
        if args.calibrate:
            prompt_inputs = get_prompt_inputs(template, eval_dataset, accelerator.device)
            model.set_prompt_prob(prompt_inputs)

        for batch in eval_dataloader:
            with torch.no_grad():
                predictions, predictions_probs, loss, pmi_dc_loss = model(batch)
            ...
            raw['prediction'].extend(predictions.cpu().tolist())
            raw['probability'].extend(predictions_probs.cpu().tolist())
            raw['loss'].append(loss.cpu().item())
            raw['pmi_dc_loss'].append(pmi_dc_loss.cpu().item())
            raw['target'].extend(batch["targets"].cpu().tolist())
            raw['accuracy'].extend((predictions == batch["targets"]).cpu().tolist())
        ...
        if accelerator.is_main_process:
            if args.output_dir is not None:
                ...
                with open(os.path.join(args.output_dir, f"{template_name}_results.json"), "w") as f:
